...
class GUI:
    ...
    def __init__(self, board_size=11):
        pygame.init()   # 1. pygame 초기화
        pygame.mixer.init()
        pygame.mixer.music.load(os.getcwd() + '/sound/game.ogg') # 음악 로드
        pygame.mixer.music.play(-1)  # 무한 루프, 플레이
    ...
    def get_input(self, current_player, end):
        """
        get inputs from clicks
        :return: variable-length array.[0] is the name. Additional information behind (maybe not exist).
        """
        count = 6 # 설정할 시간보다 +1초 해야 GUI에 이쁘게 나옴..
        while True:
            pygame.time.set_timer(24, 1000)
            event = pygame.event.wait()

            if event.type == 24 and current_player == 0 and not end:
                count -= 1
                self.show_messages(count)
                if count == 0:
                    return 'TimeOver',
